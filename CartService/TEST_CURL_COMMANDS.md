...
- **Direct Access**: Direct access to CartService (port 8084) requires `X-Gateway-Request` header with secret value (default: `gateway-secret-change-me`)
- **Cart Identity**:
  - **Authenticated users**: Cart is associated with user ID from session
  - **Anonymous users**: Cart ID is stored in `anon_cart_id` cookie
- **Product Validation**: Cart validates products against ProductService:
  - ✅ Product must exist
  - ✅ Product must be active (`isActive = true`)
  - ❌ Stock levels are NOT validated (users can add out-of-stock items)
- **Validation Rules**:
  - SKU is required (not blank)
  - Quantity must be between 1 and 999
- **Storage**: Cart data is stored in Redis for fast access
- **TTL**: Anonymous carts expire after 3 days, authenticated carts persist longer
- **Concurrency**: Cart uses optimistic locking with version numbers to prevent race conditions
- **Lua Scripts**: Add and update operations use Lua scripts for atomic Redis operations
- **Base URLs**:
  - Gateway: `http://localhost:8080`
  - CartService: `http://localhost:8084`
- **Content-Type**: All POST/PUT requests require `Content-Type: application/json`
- **Cookie Management**: HttpOnly cookies are automatically handled by curl when using `-c` (save) and `-b` (load) flags
- **Error Responses**: All errors return consistent JSON format with timestamp, status, error type, and message
- **Related Documentation**: For detailed product validation test cases, see [GatewayService/TEST_CART_VALIDATION_COMMANDS.md](../GatewayService/TEST_CART_VALIDATION_COMMANDS.md)
